...
(function($){ //anonymous function (parenthesys)
  ...
  var ListView = Backbone.View.extend({
    ...
    events: {
      ...
      //'click button#sendlogin': 'login',
      'click button.addnew': 'addItem',
      ...
      //'click td.guardar':'guardar_edicion',
      ...
      //'click th.nombre': 'sortByNombre',
      //'click th.goles1': 'sortByGoles1',
      //'click th.goles2': 'sortByGoles2',
      //'click th.pagado': 'sortByPagado'
    },
    ...
    render: function(){
      var self = this;      
      $(this.el).empty();
      var tablescaffold="<div class='participantes' style='position:absolute;margin-top:3em;'>";
      tablescaffold+="<table style='min-width:100%;'><thead>";
      tablescaffold+="<th class='tpartheader' style='width:1em;'><input type='checkbox'></input></th>";
      tablescaffold+="<th class='tpartheader' style='width:20em;'>opci&oacute;n<span>&nbsp;&nbsp;</span></th>";
      tablescaffold+="<th class='' style='width:1.2em;'>&nbsp;<span>&nbsp;&nbsp;</span></th>";
      tablescaffold+="<th class='' style='width:2em;'>&nbsp;<span>&nbsp;&nbsp;</span></th>";
      tablescaffold+="<th class='' style='width:2em;'>&nbsp;<span>&nbsp;&nbsp;</span></th>";
      tablescaffold+="<th style='width:8em;'></th>";
      tablescaffold+="<th style='width:8em;'></th>";
      tablescaffold+="</thead><tbody></tbody></table>";
      tablescaffold+="<div style='text-align:left;margin:0;padding:0;'>";
      ...
      tablescaffold+="<span> <button class='addnew' >nuevo</button> </span>";
      tablescaffold+="</div>";
      tablescaffold+="</div>";
      $(this.el).append(tablescaffold);
      _(this.collection.models).each(function(item){ 
	var style=((item.attributes.aprobado>0)?"":"background-color:grey");
        var tablecontent="<tr style='"+style+"'>";
        tablecontent+="<td><input type='checkbox' id='check_particip_"+item.attributes.position+"' "+(item.get('checked')?"checked":"")+" ></input></td>";
        tablecontent+="<td class='editar'><span class='nombre' id='nombre_part_"+item.attributes.position+"' >"+item.get('nombre')+"</span>";
        tablecontent+="</td>";
        ...
        tablecontent+="<td class='' id='position_part_"+item.attributes.position+"' >"+item.attributes.position+"</td>";
        tablecontent+="</tr>";
      ...
      $(this.el).append("<span style='display:block; width:20em;min-width:20em;max-width:20em;position:absolute;z-index:10;'><button class='addnew' >nuevo</button><span>");
    },
	...
	this.render();
	if(this.checkallChecked)$("th input[type='checkbox']",this.el).attr("checked","checked");
    },
    checkone: function(){
    ...
    movedown_checked: function(){ 
		...
		_.each(list,function(e){ 
			if(e.attributes.checked){ self.movedown_element(e); }
    		}); 
    },
    ...
    editar: function(e){
	var self=this;
	if(this.guardar_todo())this.render();
    ...
    escritura: function(event){console.debug(event);},
    guardar_edicion: function(){this.guardar_todo();this.render();},
	...
	var self=this;
	if(e.keyCode == 13) self.guardar_edicion(); 
    },
	/*
    sortByGoles1Desc:(-1),
    sortByGoles1: function(){ 
	this.guardar_todo();
        var self=this;
	this.collection.comparator=function(model){ return self.sortByGoles1Desc*(parseInt(model.attributes.goles1)); };
        this.sortByGoles1Desc=parseInt(this.sortByGoles1Desc)*(-1);
	this.collection.sort();
	this.render();
    },
    sortByGoles2Desc:(-1),
    sortByGoles2: function(){
	this.guardar_todo();
        var self=this;
        this.sortByGoles2Desc=parseInt(this.sortByGoles2Desc)*(-1);
	this.collection.comparator=function(model){ return self.sortByGoles2Desc*(parseInt(model.attributes.goles2)); };
	this.collection.sort();
	this.render();
    },
    sortByPagadoDesc:(-1),
    sortByPagado: function(){
	this.guardar_todo();
        var self=this;
        this.sortByPagadoDesc=parseInt(this.sortByPagadoDesc)*(-1);
	this.collection.comparator=function(model){ return self.sortByPagadoDesc*(parseInt(model.attributes.aprobado)?1:(-1)); };
	this.collection.sort();
	this.render();
    },
    sortByNombreDesc:(-1),
    sortByNombre: function(){ 
	//this.guardar_todo();
        var self=this;
        this.sortByNombreDesc=parseInt(this.sortByNombreDesc)*(-1);
	this.collection.comparator=function(model){ 
	 var a="0.";
	 $.each(_.map(model.attributes.nombre.split(""), function (c) { return 0xff - c.charCodeAt(); }),function(i,e){ a+=e; });
	 return self.sortByNombreDesc*parseFloat(a);
	};
	this.collection.sort();
	this.render();
    }
